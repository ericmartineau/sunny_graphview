...
class BuchheimWalkerAlgorithm extends Algorithm {
  ...
  bool isVertical() {
    ...
    return orientation == GraphOrientation.TopBottom ||
        orientation == GraphOrientation.BottomTop;
  }
  ...
  double heightTotal() {
    return levelBounds.values
        .map((e) => e.maxNodeHeight)
  ...
  bool needReverseOrder() {
    var orientation = configuration.orientation;
    return orientation == GraphOrientation.BottomTop ||
        orientation == GraphOrientation.RightLeft;
  }
  ...
  void secondWalk(Graph graph, Node node, double modifier) {
    ...
    node.position = Offset(
        ...
        (depth * (vertical ? maxHeights(depth - 1) : maxWidths(depth - 1)) +
  ...
  Offset getPosition(Node node, double globalPadding, Offset offset) {
    ...
    switch (configuration.orientation) {
      case GraphOrientation.TopBottom:
        ...
        break;
      case GraphOrientation.BottomTop:
        finalOffset = Offset(node.x - offset.dx,
            offset.dy - node.y - globalPadding - node.height);
        break;
      case GraphOrientation.LeftRight:
